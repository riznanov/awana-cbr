...
$sqlrs=mysql_query("SELECT kd_paket FROM data_paket WHERE kd_paket='$kd_paket'");
$rs=mysql_num_rows($sqlrs);
if($rs==0){
	// jika data nol maka simpan data
	$perintah="INSERT INTO data_paket (kd_paket,nama_paket,harga,kd_vendor,tema,budget) VALUES ('','$paket','$harga','$vendor','$tema', '$budget')";
	...
	if ($berhasil){
		...
		header("location: haladmin.php?top=datapaket.php");
		}else{
